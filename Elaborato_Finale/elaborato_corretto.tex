...
\chapter{Esperimenti}
\label{cap3}
Durante il tirocinio ho effettuato diversi esperimenti a partire dal dataset fornito prendendo in considerazione i vari livelli di dettaglio. In questo capitolo verranno discussi i passaggi necessari per passare dal dataset ad una previsione dell'etichetta corrispondente il più precisa possibile.
...
Iniziamo parlando degli iperparametri di SVC, in particolare di \emph{C} il cui ruolo è illustrato in Figura (ALLEGA); per un valori di \emph{C} grande è assegnata una grande PENALITA agli errori che risiedono sul margine. ciò è evidente dalla figura a sinistra di Figura 3 dove i due punti più vicini all'iperpiano influenzano il suo orientamento, ciò comporta che l'iperpiano sia più vicino agli altri punti. Considerando un \emph{C} di valore più basso, come nella figura a destra di Figura 3 questi punti diventano errori; l'orientazione dell'iperpiano è diversa rispetto a prima, inoltre c'è un un margine molto più ampio.
Un altro iperparametro di SVC è il \emph{Kernel} che può essere di diversi tipi: \emph{linear}, \emph{polynomial}, \emph{radial basis function}. Associato al kernel di tipo polinomiale c'è inoltre un altro iperparametro chiamato \emph{degree}, VEDI FIGURA ASSOCIATA
